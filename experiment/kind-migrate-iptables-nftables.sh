...
# A function that changes the kube-proxy mode
# Args:
#   $1 : kube-proxy mode: nftables, iptables
set_kubeproxy_mode() {
    local proxymode="${1:-}"
    ...
    kubectl -n kube-system rollout status ds kube-proxy --timeout=30s
}
...
main() {
    ...
    create_cluster
    # run smoke tests
    set_kubeproxy_mode "nftables"

    set_kubeproxy_mode "iptables"
