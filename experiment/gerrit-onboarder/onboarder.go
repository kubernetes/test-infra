...
func verifyInTree(workDir, host, currBranch string, configMap map[string][]string, verify func(map[string][]string) bool) (bool, error) {
	if verify(configMap) {
		return true, nil
	} else if inheritance := getInheritedRepo(configMap); inheritance != "" {
		parentBranch := currBranch + "_parent"
		...
		if err := execInDir(os.Stdout, os.Stderr, workDir, "git", "checkout", currBranch); err != nil {
			return false, fmt.Errorf("failed to checkout %s, %w", currBranch, err)
		}
...
func ensureProjectConfig(workDir, config, host, currBranch, groupName string) (string, error) {
	...
	prowReadAccess, err := verifyInTree(workDir, host, currBranch, configMap, lineInMatchingHeaderFunc(accessRefsRegex, readAccessLine))
	if err != nil {
	...
	prowLabelAccess, err := verifyInTree(workDir, host, currBranch, configMap, labelAccessExistsFunc(groupName))
	if err != nil {
		return "", fmt.Errorf("failed to check if needed lines in config: %w", err)
	}
	if !prowLabelAccess {
	...
	labelExists, err := verifyInTree(workDir, host, currBranch, configMap, labelExists)
	if err != nil {
		return "", fmt.Errorf("failed to check if needed lines in config: %w", err)
	}
	if !labelExists {
...
func updatePojectConfig(workDir, host, currBranch, groupName string) error {
	data, err := os.ReadFile(path.Join(workDir, projectConfigFile))
	if err != nil {
		return fmt.Errorf("failed to read project.config file: %w", err)
	}

	newData, err := ensureProjectConfig(workDir, string(data), host, currBranch, groupName)
	if err != nil {
