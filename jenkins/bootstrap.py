...
def pull_ref(pull):
    ...
    if isinstance(pull, basestring) and ':' in pull:
        args = [x.split(':') for x in pull.split(',')]
        refs = []
        checkouts = []
        for name, sha in args:
            try:
                num = int(name)
                refs.append('+refs/pull/%d/head:refs/pr/%d' % (num, num))
            except ValueError:  # branch spec
                refs.append(name)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--pull', help='PR number, or list of PR:sha'
                        ' pairs like master:abcd,12:ef12,45:ff65')
