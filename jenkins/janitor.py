...
# - things need to kill, in order, so I'd rather not use a dict
class resource:
...
def Collect(project, age, resource, filt):
  ...
  if resource.condition is None:
    condclause = '--format=value(name,creationTimestamp)'
  else:
    condclause = '--format=value(name,creationTimestamp,%s)' % resource.condition
...
def ClearResources(project, col, resource):
  ...
  for col, items in col.items():
    ...
    err |= subprocess.call(base + list(items))
  return err
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--filter', default="NOT tags.items:do-not-delete AND NOT name ~ ^default-route",
