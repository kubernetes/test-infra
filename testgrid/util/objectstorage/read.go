...
func ListBuilds(ctx context.Context, bkt *blob.Bucket, path Path) (Builds, error) {
	p := path.Object()
	if !strings.HasSuffix(p, "/") {
		p += "/"
	}
	it := bkt.List(&blob.ListOptions{
		Delimiter: "/",
		Prefix:    p,
	})
	var all Builds
	for {
		...
		objAttrs, err := bkt.Attributes(ctx, obj.Key)
		if err != nil {
			return nil, fmt.Errorf("failed to retrieve objects attributes: %v", err)
		}
		...
		if link := objAttrs.Metadata["link"]; len(link) > 0 {
			...
			if err != nil {
				return nil, fmt.Errorf("could not parse link for key %s: %v", objAttrs.Metadata["Name"], err)
			}
			...
			if err := linkPath.SetURL(u); err != nil {
				return nil, fmt.Errorf("could not make GCS path for key %s: %v", objAttrs.Metadata["Name"], err)
			}
		...
		if len(objAttrs.Metadata["Prefix"]) == 0 {
			continue
		}
		...
		all = append(all, Build{
			Bucket:     bkt,
			Context:    ctx,
			Prefix:     objAttrs.Metadata["Prefix"],
			BucketPath: path.Bucket(),
		})
	}
...
func readJSON(ctx context.Context, obj *blob.Bucket, uri string, i interface{}) error {
	reader, err := obj.NewReader(ctx, uri, nil)
...
func (build Build) Started() (*Started, error) {
	...
	var started Started
	err := readJSON(build.Context, build.Bucket, uri, &started)
	if gcerrors.Code(err) == gcerrors.NotFound {
...
func (build Build) Finished() (*Finished, error) {
	...
	err := readJSON(build.Context, build.Bucket, uri, &finished)
	if gcerrors.Code(err) == gcerrors.NotFound {
...
func (build Build) Artifacts(artifacts chan<- string) error {
	pref := build.Prefix
	objs := build.Bucket.List(&blob.ListOptions{Prefix: pref})
	for {
		obj, err := objs.Next(build.Context)
		if err == io.EOF {
			break
		}
		if err != nil {
...
func readSuites(ctx context.Context, obj *blob.Bucket, art string) (*junit.Suites, error) {
	reader, err := obj.NewReader(ctx, art, nil)
	if err != nil {
...
func (build Build) Suites(artifacts <-chan string, suites chan<- SuitesMeta) error {
	...
	for art := range artifacts {
		...
		go func(art string, meta map[string]string) {
			defer wg.Done()
			suitesData, err := readSuites(ctx, build.Bucket, art)
			if err != nil {
				select {
				case <-ctx.Done():
				case ec <- err:
