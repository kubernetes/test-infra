...
func Test_calcMD5(t *testing.T) {
	...
	c1 := hex.EncodeToString(calcMD5(b1))
	c2 := hex.EncodeToString(calcMD5(b2))
	c1a := hex.EncodeToString(calcMD5(b1a))

	switch {
	case c1 == c2:
		t.Errorf("c1 md5 %s should not equal c2 md5 %s", c1, c2)
	case len(b1) == 0, len(b2) == 0:
		t.Errorf("empty b1 b2 %s %s", b1, b2)
	case len(b1) != len(b1a), c1 != c1a:
		t.Errorf("different results: %s %s != %s %s", b1, c1, b1a, c1a)
	}
