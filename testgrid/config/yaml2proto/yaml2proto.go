...
func Yaml2Proto(yamlData []byte) ([]byte, error) {
	...
	for _,testgroup := range config.TestGroups {
		if !testgroup.IsExternal {
			fmt.Printf("[Warning:] Testgroup %v : is_external should always be true\n",
						testgroup.Name );
			testgroup.IsExternal = true
		}

		if !testgroup.UseKubernetesClient {
			fmt.Printf("[Warning:] Testgroup %v : use_kubernetes_client should always be true\n",
						testgroup.Name );
			testgroup.UseKubernetesClient = true
		}
	}
