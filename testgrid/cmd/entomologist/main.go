...
func (o *options) validate() error {
	...
	if outputErr := validateOutput(o.output); outputErr != nil {
		return outputErr
	}
...
func validateOutput(output string) error {
	...
	if _, err := os.Stat(output); err != nil && !strings.HasPrefix(output, "gs://") {
...
func main() {
	...
	if fi, err := os.Stat(opt.output); err == nil && fi.Mode().IsRegular() {
		outputFile = opt.output // output is already a file
	}
