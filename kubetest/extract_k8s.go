...
func (e extractStrategy) Extract(project, zone, region string, extractSrc bool) error {
	...
	case gke:
		...
		if strings.HasPrefix(e.option, "latest") {
			...
			return getKube("https://storage.googleapis.com/gke-release/kubernetes/release", version, extractSrc)
		}
	...
	case ci:
		if strings.HasPrefix(e.option, "gke-") {
			return setReleaseFromGcs("gke-release/kubernetes/release", e.option, extractSrc)
		}
	...
	case version:
		...
		if re.FindStringSubmatch(release) != nil {
			url = "https://storage.googleapis.com/gke-release/kubernetes/release"
