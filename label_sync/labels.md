...
| <a id="kind/feature" href="#kind/feature">`kind/feature`</a> | Categorizes issue or PR as related to a new feature.| anyone |  [label](https://git.k8s.io/test-infra/prow/plugins/label) |
...
| <a id="cla  human-approved" href="#cla  human-approved">`cla: human-approved`</a> | DEPRECATED. This is a holdover from when we had difficulties getting either the google or cncf CLA bots to behave. It can only be manually applied and is consumed by the submit-queue. It has seen very little use lately.| humans | |
| <a id="cla  no" href="#cla  no">`cla: no`</a> | REMOVING. This will be deleted after 2018-03-16 00:00:00 +0000 UTC <br><br> This was what we used when the googlebot was enforcing CLA. We now use the cncf-cla labels.| googlebot | |
| <a id="cla  yes" href="#cla  yes">`cla: yes`</a> | REMOVING. This will be deleted after 2018-03-16 00:00:00 +0000 UTC <br><br> This was what we used when the googlebot was enforcing CLA. We now use the cncf-cla labels.| googlebot | |
...
| <a id="do-not-merge/blocked-paths" href="#do-not-merge/blocked-paths">`do-not-merge/blocked-paths`</a> | Indicates that a PR should not be merged because it touches files in blocked paths.| prow |  [blockage](https://git.k8s.io/test-infra/prow/plugins/blockage) |
...
| <a id="do-not-merge/release-note-label-needed" href="#do-not-merge/release-note-label-needed">`do-not-merge/release-note-label-needed`</a> | Indicates that a PR should not be merged because it's missing one of the release note labels that specify whether this PR should be considered when release notes are generated.| prow |  [release-note](https://git.k8s.io/test-infra/prow/plugins/release-note) |
| <a id="do-not-merge/work-in-progress" href="#do-not-merge/work-in-progress">`do-not-merge/work-in-progress`</a> | Indicates that a PR should not be merged because it is a work in progress, based on the presence of a '[wip]' prefix in the PR title.| prow |  [wip](https://git.k8s.io/test-infra/prow/plugins/wip) |
...
| <a id="ok-to-merge" href="#ok-to-merge">`ok-to-merge`</a> | REMOVING. This will be deleted after 2018-03-16 00:00:00 +0000 UTC <br><br> This is unused.| humans | |
...
| <a id="release-note" href="#release-note">`release-note`</a> | Denotes a PR that will be considered when it comes time to generate release notes.| prow |  [release-note](https://git.k8s.io/test-infra/prow/plugins/release-note) |
| <a id="release-note-action-required" href="#release-note-action-required">`release-note-action-required`</a> | Denotes a PR that introduces potentially breaking changes that necessitate user action before upgrading to the release that includes these changes. These actions will be specifically called out when it comes time to generate release notes.| prow |  [release-note](https://git.k8s.io/test-infra/prow/plugins/release-note) |
| <a id="release-note-label-needed" href="#release-note-label-needed">`release-note-label-needed`</a> | DEPRECATED. This should be removed. Used to be added by a mungegithub munger which is no longer active. Consumed by submit-queue, but so is the replacement of `do-not-merge/release-note-label-needed`.| humans | |
| <a id="release-note-none" href="#release-note-none">`release-note-none`</a> | Denotes a PR that doesn't merit a release note, and will be ignored when it comes time to generate release notes.| prow or member or author |  [release-note](https://git.k8s.io/test-infra/prow/plugins/release-note) |
