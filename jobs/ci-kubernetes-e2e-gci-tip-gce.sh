...
# Use the latest gci-base prefixed version from container-vm-image-staging (this is the GCI canary)
export KUBE_GCE_MASTER_PROJECT="container-vm-image-staging"
export KUBE_GCE_NODE_PROJECT="container-vm-image-staging"
export KUBE_GCI_VERSION=$(gcloud compute images list --project=container-vm-image-staging -r "gci-base.*" --sort-by=~creation_timestamp | head -n2 | tail -n1 | awk '{print $1}')

export PROJECT="ci-k8s-e2e-gci-tip-gce"
