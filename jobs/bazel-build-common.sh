...
if [[ "$#" -ne 1 ]]; then
  ...
  echo
  echo "Usage: $0 <ARTIFACT_PREFIX>"
  echo
  echo "  Example: $0 gs://kubernetes-release-dev/bazel/"
  exit 1
fi

export ARTIFACT_PREFIX="$1"
...
if [[ "${rc}" == 0 ]]; then
  bazel run //:ci-artifacts -- "${ARTIFACT_PREFIX}/$(git rev-parse HEAD)" && rc=$? || rc=$?
fi
