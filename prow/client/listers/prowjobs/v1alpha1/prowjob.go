...
type ProwJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ProwJob, err error)
...
// List lists all ProwJobs in the indexer.
func (s *prowJobLister) List(selector labels.Selector) (ret []*v1alpha1.ProwJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ProwJob))
	})
	return ret, err
}
...
type ProwJobNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.ProwJob, error)
...
// List lists all ProwJobs in the indexer for a given namespace.
func (s prowJobNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ProwJob, err error) {
...
// Get retrieves the ProwJob from the indexer for a given namespace and name.
func (s prowJobNamespaceLister) Get(name string) (*v1alpha1.ProwJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("prowjob"), name)
	}
	return obj.(*v1alpha1.ProwJob), nil
}
