...
func TestOrgRepoMatchMergeMethod(t *testing.T) {
	testfile := "testdata/tide/orgrepobranchmergemethod.yaml"
	var testCases []struct {
		...
		Expected types.PullRequestMergeType `json:"expected"`
	}
	testdata, err := os.ReadFile(testfile)
	if err != nil {
		t.Fatalf("read %s: %s", testfile, err.Error())
	}
	if err := yaml.Unmarshal(testdata, &testCases); err != nil {
		t.Fatalf("unmarshal %s: %s", testfile, err.Error())
	}
	for _, test := range testCases {
		test := test
		t.Run(test.Name, func(t *testing.T) {
			if err := parseTideMergeType(test.Tide.MergeType); err != nil {
				t.Fatalf("parse tide merge type: %s", err.Error())
			}
			actual := test.Tide.OrgRepoBranchMergeMethod(OrgRepo{Org: test.Org, Repo: test.Repo}, test.Branch)
			if actual != test.Expected {
