...
func helpProvider(config *plugins.Configuration, enabledRepos []string) (*pluginhelp.PluginHelp, error) {
	return &pluginhelp.PluginHelp{
			Description: fmt.Sprintf("The verify-owners plugin validates %s files if they are modified in a PR. On validation failure it automatically adds the '%s' label to the PR, and a review comment on the incriminating file(s).", ownersFileName, labels.InvalidOwnersLabel),
		},
		nil
}
...
func handle(ghc githubClient, gc *git.Client, log *logrus.Entry, pre *github.PullRequestEvent, labelsBlackList []string) error {
	...
	if len(wrongOwnersFiles) > 0 {
		...
		if err := ghc.AddLabel(org, repo, pre.Number, labels.InvalidOwnersLabel); err != nil {
			return err
		}
		...
		labelNotFound := true
		if err := ghc.RemoveLabel(org, repo, pre.Number, labels.InvalidOwnersLabel); err != nil {
			if _, labelNotFound = err.(*github.LabelNotFound); !labelNotFound {
				return fmt.Errorf("failed removing %s label: %v", labels.InvalidOwnersLabel, err)
			}
