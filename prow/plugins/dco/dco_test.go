...
`,
		...
		{
			name:   "should update URL with non-master default branch",
			config: plugins.Dco{},
			pullRequestEvent: github.PullRequestEvent{
				Action:      github.PullRequestActionOpened,
				PullRequest: github.PullRequest{Number: 3, Head: github.PullRequestBranch{SHA: "sha"}},
...
:memo: **Please follow instructions in the [contributing guide](https://github.com///blob/other-default/CONTRIBUTING.md) to update your commits with the DCO**
...
`,
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.defaultBranch != "" {
				fc.DefaultBranch = tc.defaultBranch
			}
			if tc.hasDCOYes {
