...
<li><a href="get.php">get.php</a> , type x-mapp-php5</li>
	...
	if _, err := handle(fc, logrus.WithField("plugin", pluginName), e, &realClowder{url: ts.URL + "/?format=json"}, func() {}); err != nil {
...
func TestCats(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		_, err := handle(fc, logrus.WithField("plugin", pluginName), e, fakeClowder("tubbs"), func() {})
