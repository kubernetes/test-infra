...
func TestIsApprovedWithIssue(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName:          "Single file. With issue. File approved. Issue approval does not cover file",
			filenames:         []string{"a/file.go"},
		...
		{
			testName:          "Two files. With Issue. Files partially approved. Issue Approval covers remainder of unapproved files",
			filenames:         []string{"a/file.go", "b/file2.go"},
			currentlyApproved: []approval{{"Carl", "a/*"}},
			noIssueApprovers:  sets.NewString("Bill"),
			associatedIssue:   100,
