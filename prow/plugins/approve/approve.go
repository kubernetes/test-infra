...
func handlePullRequest(log *logrus.Entry, ghc githubClient, oc ownersClient, config *plugins.Configuration, pre *github.PullRequestEvent) error {
	...
	if pre.Action == github.PullRequestActionLabeled &&
		(pre.Label.Name != labels.ApprovedLabel || pre.Sender.Login == botName || pre.PullRequest.State == "closed") {
		return nil
	}
...
func handle(log *logrus.Entry, ghc githubClient, repo approvers.RepoInterface, opts *plugins.Approve, pr *state) error {
	...
	for _, label := range issueLabels {
		if label.Name == labels.ApprovedLabel {
	...
	if !approversHandler.IsApproved() {
		if hasApprovedLabel {
			if err := ghc.RemoveLabel(pr.org, pr.repo, pr.number, labels.ApprovedLabel); err != nil {
				log.WithError(err).Errorf("Failed to remove %q label from %s/%s#%d.", labels.ApprovedLabel, pr.org, pr.repo, pr.number)
			}
		}
	} else if !hasApprovedLabel {
		if err := ghc.AddLabel(pr.org, pr.repo, pr.number, labels.ApprovedLabel); err != nil {
			log.WithError(err).Errorf("Failed to add %q label to %s/%s#%d.", labels.ApprovedLabel, pr.org, pr.repo, pr.number)
		}
	}
	return nil
}
...
func humanAddedApproved(ghc githubClient, log *logrus.Entry, org, repo string, number int, botName string, hasLabel bool) func() bool {
	findOut := func() bool {
		...
		for _, event := range events {
			...
			if event.Event != github.IssueActionLabeled || event.Label.Name != labels.ApprovedLabel {
				continue
			}
			lastAdded = event
		}
