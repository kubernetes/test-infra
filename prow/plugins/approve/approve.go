...
func handle(log *logrus.Entry, ghc githubClient, repo approvers.Repo, githubConfig config.GitHubOptions, opts *plugins.Approve, pr *state) error {
	...
	newMessage := updateNotification(githubConfig.LinkURL, opts.CommandHelpLink, opts.PrProcessLink, pr.org, pr.repo, pr.branch, repo.StaleRepositoryState(), latestNotification, approversHandler)
...
func updateNotification(linkURL *url.URL, commandHelpLink, prProcessLink, org, repo, branch, staleRepositoryState string, latestNotification *comment, approversHandler approvers.Approvers) *string {
	message := approvers.GetMessage(approversHandler, linkURL, commandHelpLink, prProcessLink, org, repo, branch, staleRepositoryState)
