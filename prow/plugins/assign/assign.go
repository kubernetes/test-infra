...
func handleGenericComment(pc plugins.Agent, e github.GenericCommentEvent) (plugins.Status, error) {
	...
	err := handle(newAssignHandler(e, pc.GitHubClient, pc.Logger), &status)
	if e.IsPR {
		err = combineErrors(err, handle(newReviewHandler(e, pc.GitHubClient, pc.Logger), &status))
	}
	return status, err
}
...
func handle(h *handler, status *plugins.Status) error {
	e := h.event
	...
	if len(toRemove) > 0 {
		...
		err := h.remove(org, repo, e.Number, toRemove)
		status.TookAction()
		if err != nil {
			return err
		}
	...
	if len(toAdd) > 0 {
		...
		err := h.add(org, repo, e.Number, toAdd)
		status.TookAction()
		if err != nil {
