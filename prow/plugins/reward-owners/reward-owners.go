...
func helpProvider(_ *plugins.Configuration, _ []config.OrgRepo) (*pluginhelp.PluginHelp, error) {
	pluginHelp := &pluginhelp.PluginHelp{
		Description: fmt.Sprintf("The reward-owners plugin watches in %s and %s files for modifications and welcomes new approvers and reviewers.", ownersFileName, ownersAliasesFileName),
	}
...
func handlePullRequest(pc plugins.Agent, pre github.PullRequestEvent) error {
	...
	return handle(pc.GitHubClient, pc.GitClient, pc.Logger, &pre.PullRequest, prInfo, pc.PluginConfig.TriggerFor(pre.Repo.Owner.Login, pre.Repo.Name), skipTrustedUserCheck)
}

func handle(ghc githubClient, gc git.ClientFactory, log *logrus.Entry, pr *github.PullRequest, info info, triggerConfig plugins.Trigger, skipTrustedUserCheck bool) error {
	org := info.org
	repo := info.repo
	number := info.number
	...
	for _, change := range changes {
		if change.Status != github.PullRequestFileRemoved {
			if filepath.Base(change.Filename) == ownersFileName || filepath.Base(change.Filename) == ownersAliasesFileName {
	...
	_, _, repoAliases, err := verifyowners.NonTrustedUsersInOwnersAliases(ghc, log, triggerConfig, org, repo, r.Directory(), modifiedOwnerAliasesFile.Patch, ownerAliasesModified, skipTrustedUserCheck)
	if err != nil {
		return err
	}
