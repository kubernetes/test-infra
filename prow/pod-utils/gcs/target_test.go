...
func TestPathForSpec(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "postsubmit",
			spec: &decorate.JobSpec{Type: kube.PostsubmitJob,
				Job:     "job",
				BuildId: "number",
			},
			expected: "logs/job/number",
		...
		{
			name: "batch",
			spec: &decorate.JobSpec{Type: kube.BatchJob,
				Job:     "job",
				BuildId: "number",
			},
...
func TestAliasForSpec(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "periodic",
			spec:     &decorate.JobSpec{Type: kube.PeriodicJob},
			expected: "",
		},
		{
			name:     "batch",
...
func TestLatestBuildForSpec(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "batch",
			spec:     &decorate.JobSpec{Type: kube.BatchJob, Job: "pull-kubernetes-unit"},
			expected: []string{"pr-logs/directory/pull-kubernetes-unit/latest-build.txt"},
		},
		{
			name:     "postsubmit",
			spec:     &decorate.JobSpec{Type: kube.PostsubmitJob, Job: "ci-kubernetes-unit"},
			expected: []string{"logs/ci-kubernetes-unit/latest-build.txt"},
		},
		{
			name:     "periodic",
			spec:     &decorate.JobSpec{Type: kube.PeriodicJob, Job: "ci-kubernetes-periodic"},
...
func TestRootForSpec(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "presubmit",
			spec:     &decorate.JobSpec{Type: kube.PresubmitJob, Job: "pull-kubernetes-unit"},
