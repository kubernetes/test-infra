...
>>>>>>> cbf2bd5... work
	...
	if !pj.Spec.Decorate {
		...
		gcsCredentialsVolume := kube.Volume{
			...
			VolumeSource: kube.VolumeSource{
				Secret: &kube.SecretSource{
					SecretName: pj.Spec.GCSCredentialsSecret,
				},
			},
		}
		...
		cloneConfigEnv, err := clonerefs.Encode(clonerefs.Options{
			...
			GitRefs:      append(pj.Spec.ExtraRefs, pj.Spec.Refs),
		})
		if err != nil {
		...
		gcsOptions := gcsupload.Options{
			...
			GCSConfiguration:   pj.Spec.GCSConfiguration,
		...
		spec.InitContainers = []kube.Container{
			{
				Name:         "clonerefs",
				Image:        pj.Spec.UtilityImages.CloneRefs,
			...
			{
				Name:    "initupload",
				Image:   pj.Spec.UtilityImages.InitUpload,
			...
			{
				Name:         "place-tools",
				Image:        pj.Spec.UtilityImages.Entrypoint,
		...
		entrypointConfigEnv, err := entrypoint.Encode(entrypoint.Options{
			...
			TimeoutMinutes: pj.Spec.TimeoutMinutes,
		})
		if err != nil {
		...
		spec.Containers = append(spec.Containers, kube.Container{
			Name:    "sidecar",
			Image:   pj.Spec.UtilityImages.Sidecar,
