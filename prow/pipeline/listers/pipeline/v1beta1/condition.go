...
type ConditionLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Condition, err error)
...
// List lists all Conditions in the indexer.
func (s *conditionLister) List(selector labels.Selector) (ret []*v1beta1.Condition, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Condition))
	})
	return ret, err
}
...
type ConditionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Condition, error)
...
// List lists all Conditions in the indexer for a given namespace.
func (s conditionNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Condition, err error) {
...
// Get retrieves the Condition from the indexer for a given namespace and name.
func (s conditionNamespaceLister) Get(name string) (*v1beta1.Condition, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("condition"), name)
	}
	return obj.(*v1beta1.Condition), nil
}
