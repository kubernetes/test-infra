...
func main() {
	...
	repo := githubRepoRE.FindString(*remoteURL)
	...
	for {
		...
		log.Println("PRs in queue:", queue)
		if err != nil {
			log.Printf("error getting queued PRs: %v\n", err)
			time.Sleep(1 * time.Minute)
			continue
		}
		batchPRs, err := splicer.findMergeable(*remoteURL, queue)
		if err != nil {
			log.Printf("error computing mergeable PRs: %v\n", err)
			time.Sleep(1 * time.Minute)
			continue
		}
		buildRef := splicer.makeBuildRef(repo, batchPRs)
		buf, err := json.Marshal(buildRef)
		if err != nil {
			log.Fatal("unable to marshal JSON", err)
		}
		log.Println("batch PRs:", batchPRs)
		log.Println("batch buildRef:", string(buf))
		time.Sleep(1 * time.Minute)
	}
}
