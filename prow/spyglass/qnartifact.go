...
// QNArtifact represents some output of a prow job stored in qiniu cloud
type QNArtifact struct {
	// The handle of the object in qiniu cloud
	handle qnArtifactHandle
...
type qnArtifactHandle interface {
...
// NewQNArtifact returns a new GCSArtifact with a given handle, canonical link, and path within the job
func NewQNArtifact(ctx context.Context, handle qnArtifactHandle, link string, path string, sizeLimit int64) *QNArtifact {
	return &QNArtifact{
		handle:    handle,
		link:      link,
		path:      path,
...
func (a *QNArtifact) ReadAtMost(n int64) ([]byte, error) {
...
func (a *QNArtifact) ReadAll() ([]byte, error) {
	size, err := a.Size()
	if err != nil {
		return nil, fmt.Errorf("error getting artifact size: %v", err)
	}
...
func (a *QNArtifact) ReadTail(n int64) ([]byte, error) {
	gzipped, err := a.gzipped()
	if err != nil {
		return nil, fmt.Errorf("error checking artifact for gzip compression: %v", err)
	}
