...
func TestInteractor_MergeWithStrategy(t *testing.T) {
	var testCases = []struct {
		...
		},
		{
			...
			strategy:   "squash",
			responses: map[string]execResponse{
				"merge --squash --no-stat -m merge shasum": {
					out: []byte(`ok`),
				},
			},
			expectedCalls: [][]string{
...
func TestInteractor_MergeAndCheckout(t *testing.T) {
	var testCases = []struct {
		...
		},
		{
			...
			responses: map[string]execResponse{
				...
				"merge --squash --no-stat -m merge first": {
					out: []byte(`ok`),
				},
				"merge --squash --no-stat -m merge second": {
					out: []byte(`ok`),
				},
			},
			expectedCalls: [][]string{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			actualErr := i.MergeAndCheckout(testCase.baseSHA, testCase.commitlikes, testCase.strategy)
			if testCase.expectedErr && actualErr == nil {
				t.Errorf("%s: expected an error but got none", testCase.name)
			}
			if !testCase.expectedErr && actualErr != nil {
