...
func (i *interactor) Clone(from string) error {
	...
	if out, err := i.executor.Run("clone", i.dir, from); err != nil {
...
func (i *interactor) MergeWithStrategy(commitlike, mergeStrategy string) (bool, error) {
	...
	out, err := i.executor.Run("merge", mergeFlag, "--no-stat", "-m", "merge", commitlike)
	if err == nil {
		return true, nil
	}
...
func (i *interactor) MergeAndCheckout(baseSHA string, headSHAs []string, mergeStrategy string) error {
	if baseSHA == "" {
