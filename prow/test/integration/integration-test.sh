...
main() {
  ...
  (rerun 3 "bazel run //prow:testimage-push  "$@"") || ( echo "FAILED: pushing images">&2; return 1 )
  bazel run //prow/test/integration:setup-cluster "$@" || ( echo "FAILED: setup cluster">&2; return 1 )
  bazel run //prow/test/integration:run-test --test_arg=--run-integration-test "$@" || ( echo "FAILED: running tests">&2; return 1 )
}
