...
if ! which bazel >/dev/null 2>&1 || [[ "$(bazel --version)" != "bazel 4.0.0" ]]; then
  echo "Install bazel for prow"
  mkdir -p "/usr/local/lib/bazel/bin"
  pushd "/usr/local/lib/bazel/bin" >/dev/null
  curl -LO https://releases.bazel.build/3.0.0/release/bazel-4.0.0-linux-x86_64
  chmod +x bazel-4.0.0-linux-x86_64
  popd
fi
