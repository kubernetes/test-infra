...
func TestRequirementConstructor(t *testing.T) {
	requirementConstructorTests := []struct {
		...
		{"x", In, nil, false},
		{"x", NotIn, sets.NewString(), false},
		{"x", In, sets.NewString("foo"), true},
		{"x", NotIn, sets.NewString("foo"), true},
		{"x", Exists, nil, true},
		{"x", DoesNotExist, nil, true},
		{"1foo", In, sets.NewString("bar"), true},
		{"1234", In, sets.NewString("bar"), true},
		{"y", GreaterThan, sets.NewString("1"), true},
		{"z", LessThan, sets.NewString("6"), true},
		{"foo", GreaterThan, sets.NewString("bar"), false},
		{"barz", LessThan, sets.NewString("blah"), false},
		{strings.Repeat("a", 254), Exists, nil, false}, //breaks DNS rule that len(key) <= 253
	}
...
func TestToString(t *testing.T) {
	...
	toStringTests := []struct {
		...
		{&internalSelector{
			getRequirement("x", In, sets.NewString("abc", "def"), t),
			getRequirement("y", NotIn, sets.NewString("jkl"), t),
			getRequirement("z", Exists, nil, t)},
			"x in (abc,def),y notin (jkl),z", true},
		{&internalSelector{
			getRequirement("x", NotIn, sets.NewString("abc", "def"), t),
			getRequirement("y", NotEquals, sets.NewString("jkl"), t),
			getRequirement("z", DoesNotExist, nil, t)},
		...
		{&internalSelector{
			...
			"x in (abc,def),", false},
		{&internalSelector{
			getRequirement("x", NotIn, sets.NewString("abc"), t),
			getRequirement("y", In, sets.NewString("jkl", "mno"), t),
			getRequirement("z", NotIn, sets.NewString(""), t)},
			"x notin (abc),y in (jkl,mno),z notin ()", true},
		{&internalSelector{
			getRequirement("x", Equals, sets.NewString("abc"), t),
			getRequirement("y", DoubleEquals, sets.NewString("jkl"), t),
			getRequirement("z", NotEquals, sets.NewString("a"), t),
			getRequirement("z", Exists, nil, t)},
			"x=abc,y==jkl,z!=a,z", true},
		{&internalSelector{
			getRequirement("x", GreaterThan, sets.NewString("2"), t),
			getRequirement("y", LessThan, sets.NewString("8"), t),
			getRequirement("z", Exists, nil, t)},
			"x>2,y<8,z", true},
	}
...
func TestRequirementSelectorMatching(t *testing.T) {
	...
	labelSelectorMatchingTests := []struct {
		...
		{Set{"x": "foo", "y": "baz"}, &internalSelector{
			getRequirement("x", In, sets.NewString("foo"), t),
			getRequirement("y", NotIn, sets.NewString("alpha"), t),
		}, true},
		{Set{"x": "foo", "y": "baz"}, &internalSelector{
			getRequirement("x", In, sets.NewString("foo"), t),
			getRequirement("y", In, sets.NewString("alpha"), t),
		}, false},
		{Set{"y": ""}, &internalSelector{
			getRequirement("x", NotIn, sets.NewString(""), t),
			getRequirement("y", Exists, nil, t),
		}, true},
		{Set{"y": ""}, &internalSelector{
			getRequirement("x", DoesNotExist, nil, t),
			getRequirement("y", Exists, nil, t),
		}, true},
		{Set{"y": ""}, &internalSelector{
			getRequirement("x", NotIn, sets.NewString(""), t),
			getRequirement("y", DoesNotExist, nil, t),
		}, false},
		{Set{"y": "baz"}, &internalSelector{
			getRequirement("x", In, sets.NewString(""), t),
		}, false},
		{Set{"z": "2"}, &internalSelector{
			getRequirement("z", GreaterThan, sets.NewString("1"), t),
		}, true},
...
func TestSetSelectorParser(t *testing.T) {
	setSelectorParserTests := []struct {
		...
		{"this-is-a-dns.domain.com/key-with-dash", internalSelector{
			getRequirement("this-is-a-dns.domain.com/key-with-dash", Exists, nil, t),
		}, true, true},
		{"this-is-another-dns.domain.com/key-with-dash in (so,what)", internalSelector{
			getRequirement("this-is-another-dns.domain.com/key-with-dash", In, sets.NewString("so", "what"), t),
		}, true, true},
		{"0.1.2.domain/99 notin (10.10.100.1, tick.tack.clock)", internalSelector{
			getRequirement("0.1.2.domain/99", NotIn, sets.NewString("10.10.100.1", "tick.tack.clock"), t),
		}, true, true},
		{"foo  in	 (abc)", internalSelector{
			getRequirement("foo", In, sets.NewString("abc"), t),
		}, true, true},
		...
		{"x in (abc,)", internalSelector{
			getRequirement("x", In, sets.NewString("abc", ""), t),
		}, true, true},
		...
		{"x notin (abc,,def),bar,z in (),w", internalSelector{
			...
			getRequirement("x", NotIn, sets.NewString("abc", "", "def"), t),
			getRequirement("z", In, sets.NewString(""), t),
		}, true, true},
		{"x,y in (a)", internalSelector{
			getRequirement("y", In, sets.NewString("a"), t),
			getRequirement("x", Exists, nil, t),
		}, false, true},
		{"x=a", internalSelector{
			getRequirement("x", Equals, sets.NewString("a"), t),
		}, true, true},
		{"x>1", internalSelector{
			getRequirement("x", GreaterThan, sets.NewString("1"), t),
		}, true, true},
		{"x<7", internalSelector{
			getRequirement("x", LessThan, sets.NewString("7"), t),
		}, true, true},
		{"x=a,y!=b", internalSelector{
			getRequirement("x", Equals, sets.NewString("a"), t),
			getRequirement("y", NotEquals, sets.NewString("b"), t),
		}, true, true},
		{"x=a,y!=b,z in (h,i,j)", internalSelector{
			getRequirement("x", Equals, sets.NewString("a"), t),
			getRequirement("y", NotEquals, sets.NewString("b"), t),
			getRequirement("z", In, sets.NewString("h", "i", "j"), t),
		}, true, true},
		...
		{"x notin (, a)", internalSelector{
			getRequirement("x", NotIn, sets.NewString("", "a"), t),
		}, true, true},
		{"a in (xyz),", nil, true, false},
		{"a in (xyz)b notin ()", nil, true, false},
		{"a ", internalSelector{
			getRequirement("a", Exists, nil, t),
		}, true, true},
		{"a in (x,y,notin, z,in)", internalSelector{
			getRequirement("a", In, sets.NewString("in", "notin", "x", "y", "z"), t),
...
func TestAdd(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			In,
			[]string{"value"},
			internalSelector{Requirement{"key", In, []string{"value"}}},
		},
		{
			"keyEqualsOperator",
			internalSelector{Requirement{"key", In, []string{"value"}}},
			...
			[]string{"value2"},
			internalSelector{
				Requirement{"key", In, []string{"value"}},
				Requirement{"key2", Equals, []string{"value2"}},
			},
		},
	}
