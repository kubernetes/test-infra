...
// This is a copy of https://github.com/kubernetes/apimachinery/blob/18a564baac720819100827c16fdebcadb05b2d0d/pkg/labels/labels_test.go
...
func TestLabelMerge(t *testing.T) {
	...
	for _, test := range tests {
		...
		if !Equal(mergedLabels, test.mergedLabels) {
...
func TestLabelSelectorParse(t *testing.T) {
	...
	for _, test := range tests {
		...
		if !Equal(Set(labels), test.labels) {
