...
func (o Options) doUpload(spec *downwardapi.JobSpec, passed, aborted bool, metadata map[string]interface{}, logReader io.Reader) error {
	uploadTargets := map[string]qiniu.UploadFunc{
		"build-log.txt": qiniu.DataUpload(logReader),
	}
	...
	if err != nil {
		...
		uploadTargets["finished.json"] = qiniu.DataUpload(bytes.NewBuffer(finishedData))
	}

	if err := o.QnOptions.Run(spec, uploadTargets); err != nil {
