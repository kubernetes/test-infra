...
import (
	...
	ctruntimeclient "sigs.k8s.io/controller-runtime/pkg/client"
...
func TestTerminateDupProwJobs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := buildClient.List(context.Background(), &ctruntimeclient.ListOptions{Namespace: fakePJNS}, buildList); err != nil {
