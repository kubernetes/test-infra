...
def main():
    ...
    if os.getenv('DEPLOYMENT', 'staging') == "prod":
      call(f'{mj_cmd} {mj_ext} --days {DAY} | jq -c \'.[]\' | pv | gzip > build_day.json.gz')
        call(f'{bq_cmd} {bq_ext} k8s-gubernator:build.day build_day.json.gz schema.json')

        call(f'{mj_cmd} {mj_ext} --days {WEEK} | jq -c \'.[]\' | pv | gzip > build_week.json.gz')
        ...
        call(f'{mj_cmd} --days {MONTH} | jq -c \'.[]\' | pv | gzip > build_all.json.gz')
        ...
        call(f'{mj_cmd} | jq -c \'.[]\' | pv | gzip > build_staging.json.gz')
