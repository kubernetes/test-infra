...
#    curl -L https://github.com/kubernetes-sigs/kind/raw/main/hack/ci/e2e-k8s.sh | diff -c - hack/ci/test-infra-e2e-k8s.sh
#
# This version has no defaults which restrict which tests are running.
# The job is in full control. Setting LABEL_FILTER is the recommended
# approach.
#
# Must be run with a Kubernetes checkout in $PWD (IE from the checkout).
#
# Usage: LABEL_FILTER=<Ginkgo label filter> kind-e2e.sh

set -o errexit -o nounset -o xtrace

# Settings:
# LABEL_FILTER:
#           Ginkgo label query for selecting tests (see "Spec Labels" in https://onsi.github.io/ginkgo/#filtering-specs)
...
#
# See also GINKGO_PARALLEL_NODES and other GINKGO_* env variables in
# in https://github.com/kubernetes/kubernetes/blob/master/hack/ginkgo-e2e.sh
#
# Legacy settings:
