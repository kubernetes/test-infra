...
#    keeping the original, corresponding k8s.io/kubernetes via a "Kubernetes-commit: <kube-sha>"
...
#   │─┘
#   I─┐ Merge pull request #50562 from atlassian/call-cleanup-properly
#   │ o Call the right cleanup function
#   │─┘
#   I─┐ Merge pull request #51154 from RenaudWasTaken/gRPC-updated-1-3-0
...
sync_repo() {
	...
	# new master branch
	filter-branch ${src_branch} "${subdirectory}" filtered-branch

	# find commits on the main line (mostly merges, some commits due to fast-forward merges that filter-branch dropped)
	...
	if [ "${new_branch}" = "true" ]; then
	    ...
	    filter-branch "${subdirectory}" filtered-branch filtered-branch-base
            ...
            local k_branch_point_commit=$(kube-commit filtered-branch-base) # this will probably different than the k_branch_point_commit above because filtered drops commits and maps to ancestors if necessary
	...
	if [ ${f_mainline_commit} = FLUSH_PENDING_MERGE_COMMIT ]; then
		...
		if [ -n "${f_pending_merge_commit}" ]; then
		    echo "Cherry-picking k8s.io/kubernetes master-merge  ${k_pending_merge_commit}: $(git show --format="%s" -q ${k_pending_merge_commit})."
		    ...
		    dst_needs_godeps_update=true
                    local date=$(git show --format="%aD" -q ${f_pending_merge_commit})
                    if ! GIT_COMMITTER_DATE="${date}" git cherry-pick --keep-redundant-commits -m 1 ${f_pending_merge_commit} >/dev/null; then
			echo
		    ...
		    echo "Cherry-picking k8s.io/kubernetes dropped-master-merge ${k_pending_merge_commit}: $(git show --format="%s" -q ${k_pending_merge_commit})."
		    git reset -q --hard ${dst_parent2}
		fi
	    else
		echo "Cherry-picking k8s.io/kubernetes dropped-merge ${k_pending_merge_commit}: $(git show --format="%s" -q ${k_pending_merge_commit})."
	    fi
	    local date=$(git show --format="%aD" -q ${k_pending_merge_commit}) # author and committer date is equal for PR merges
	    local dst_new_merge=$(GIT_COMMITTER_DATE="${date}" GIT_AUTHOR_DATE="${date}" git commit-tree -p ${dst_merge_point_commit} -p ${dst_parent2} -m "$(git show --format='%B' -q ${k_pending_merge_commit}; echo; echo "Kubernetes-commit: ${k_pending_merge_commit}")" HEAD^{tree})
	...
	if [ ${dst_branch} != master ] && is-merge-with-master ${k_mainline_commit}; then
	    echo "Deferring master merge commit ${k_mainline_commit}: $(git show --format="%s" -q ${f_mainline_commit})."
	elif [ ${dst_branch} != master ] && [ -n "${k_pending_merge_commit}" ] && is-merge-with-master "${k_pending_merge_commit}"; then
	    echo "Skipping master commit ${k_mainline_commit}: $(git show --format="%s" -q ${f_mainline_commit}). Master merge commit ${k_pending_merge_commit} is pending."
	elif ! git show -q ${f_mainline_commit} | grep -q "^Merge: "; then
            echo "Cherry-picking k8s.io/kubernetes single-commit ${k_mainline_commit}: $(git show --format="%s" -q ${f_mainline_commit})."

            # reset Godeps.json?
            local squash_commits=1
            if [ -n "$(godep-changes ${f_mainline_commit})" ]; then
            ...
            # finally cherry-pick
            local date=$(git show --format="%aD" -q ${f_mainline_commit})
            if ! GIT_COMMITTER_DATE="${date}" git cherry-pick --keep-redundant-commits ${f_mainline_commit} >/dev/null; then
                echo
                show-working-dir-status
                return 1
            fi
            ...
            if [ -n "${f_latest_merge_commit}" ]; then
                echo "Cherry-picking squashed k8s.io/kubernetes branch-commits $(kube-commit ${f_latest_branch_point_commit})..$(kube-commit ${f_latest_merge_commit}) because the last one is a merge: $(git show --format="%s" -q ${f_latest_merge_commit})"

                # reset Godeps.json?
                local squash_commits=1
                if [ -n "$(godep-changes ${f_latest_branch_point_commit} ${f_latest_merge_commit})" ]; then
                ...
                git commit -q -m "sync: squashed up to merge $(kube-commit ${f_latest_merge_commit}) in ${k_mainline_commit}"
            ...
            for f_commit in $(git log --format='%H' --reverse ${f_first_pick_base}..${f_mainline_commit}^2); do
                # reset Godeps.json?
                local squash_commits=1
                if [ -n "$(godep-changes ${f_commit})" ]; then
                    reset-godeps ${f_commit}^
                    squash_commits=2 # squash the cherry-pick into the godep reset commit below
                    dst_needs_godeps_update=true
                fi

                echo "Cherry-picking k8s.io/kubernetes branch-commit $(kube-commit ${f_commit}): $(git show --format="%s" -q ${f_commit})."
                local date=$(git show --format="%aD" -q ${f_commit})
                if ! GIT_COMMITTER_DATE="${date}" git cherry-pick --keep-redundant-commits ${f_commit} >/dev/null; then
                    echo
                    show-working-dir-status
                    return 1
                fi
            ...
            echo "Cherry-picking k8s.io/kubernetes branch-merge  ${k_mainline_commit}: $(git show --format="%s" -q ${f_mainline_commit})."
            local date=$(git show --format="%aD" -q ${f_mainline_commit}) # author and committer date is equal for PR merges
            git reset -q $(GIT_COMMITTER_DATE="${date}" GIT_AUTHOR_DATE="${date}" git commit-tree -p ${dst_merge_point_commit} -p HEAD -m "$(git show --format="%B" -q ${f_mainline_commit})" HEAD^{tree})
            fix-godeps "${deps}" "${is_library}" ${dst_needs_godeps_update}
            dst_needs_godeps_update=false
            dst_merge_point_commit=$(git rev-parse HEAD)
        fi
...
function is-merge-with-master() {
    if ! grep -q "^Merge remote-tracking branch 'origin/master'" <<<"$(git show --format="%s" -q ${1})"; then
	return 1
    fi
}
...
function godep-changes() {
    if [ -n "${2:-}" ]; then
        git diff ${1} ${2} -- Godeps/Godeps.json
    else
        git diff ${1}^ ${1} -- Godeps/Godeps.json
    fi
}
...
function kube-commit() {
    git show --format="%B" -q ${1:-HEAD} | grep "^Kubernetes-commit: " | sed "s/^Kubernetes-commit: //g"
}
...
function squash() {
    ...
    git commit --allow-empty -q -m "$(git show --format="%B" -q ${head})"
}
