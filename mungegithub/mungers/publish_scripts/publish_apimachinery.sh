...
# This script publishes the latest changes in
# k8s.io/kubernetes/staging/src/apimachinery to the k8s.io/apimachinery.
# The expected working directory is $GOPATH/src/k8s.io/apimachinery/

if [ ! $# -eq 2 ]; then
    echo "usage: apimachinery-sync-from-kubernetes.sh token"
    exit 1
fi
...
sync() {
    ...
    git -c user.name="Kubernetes Publisher" -c user.email
    "k8s-publish-robot@users.noreply.github.com" commit -m "sync(k8s.io/kubernetes): ${newKubeSHA}" -- kubernetes-sha filter-branch-sha
}
