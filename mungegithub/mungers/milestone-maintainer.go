...
const (
	...
	// Warn about milestone-required labels every day
	milestoneWarningPeriod = 1 * day
...
func (m *MilestoneMaintainer) RegisterOptions(opts *options.Options) sets.String {
	...
	opts.RegisterInt(&m.gracePeriodDays, "milestone-grace-period", 3, "The grace period (in days) to wait before removing an incomplete issue from the active milestone.")
	opts.RegisterUpdateCallback(func(changed sets.String) error {
		if changed.HasAny("active-milestone") {
			if len(m.activeMilestone) == 0 {
				return errors.New("active-milestone must be supplied")
			}
		}
		if changed.HasAny("milestone-grace-period") {
			if m.gracePeriodDays <= 0 {
				return errors.New("milestone-grace-period must be greater than zero")
			}
		}
...
func (m *MilestoneMaintainer) Munge(obj *github.MungeObject) {
	...
	gracePeriod := day * time.Duration(m.gracePeriodDays)
	labelName, notifyState := notificationState(obj, comment, m.botName, gracePeriod)
...
func notificationState(obj *github.MungeObject, comment *c.Comment, botName string, gracePeriod time.Duration) (string, *milestoneNotification) {
	...
	if isBlocker || *removeAfter >= 0 {
		...
		return milestoneLabelsIncompleteLabel, labelsIncompleteState(obj.Issue, notification, labelErrors, createdAt, removeAfter)
	}
...
func labelsIncompleteState(issue *githubapi.Issue, notification *c.Notification, labelErrors []string, commentCreatedAt *time.Time, removeAfter *time.Duration) *milestoneNotification {
	const template = `{{.mention}}
...
`
	...
	isCurrent := (notification != nil && notification.Arguments == milestoneLabelsIncomplete && (commentCreatedAt == nil || time.Since(*commentCreatedAt) < milestoneWarningPeriod))
	if isCurrent {
		return nil
	}
