...
_k8s_secret = rule(
  ...
  attrs={
    ...
    "_writer": attr.label(executable=True, cfg="host", allow_files=True,
			  default=Label("//def/secret")),
  },
)

# A macro to create a secret object as well as rules to manage it.
#
# Usage:
#   k8s_secret("something", data={"foo": "//path/to/foo.json"})
#
# This is roughly equivalent to:
#    kubectl create secret something --from-file=foo=path/to/foo.json
# Supports cluster=kubectl_context, namespace="blah", labels={"app": "fancy"}
# as well as any args k8s_object supports.
# 
# Generates a k8s_object(kind="secret") with the generated template.
#
# See also:
#   * https://docs.bazel.build/versions/master/skylark/macros.html
#   * https://github.com/bazelbuild/rules_k8s#k8s_object
def k8s_secret(name, data=None, namespace='', labels=None, cluster='', **kw):
  # Create the non-duplicated list of data values
  _data = data or {}
  _data_targets = {v: None for v in _data.values()}.keys()
  # Create the rule to generate the secret
  _k8s_secret(
    name = name + ".generated-yaml",
    data=data,
    namespace=namespace,
    labels=labels,
    output=name + "_secret.yaml",
    data_strings=_data_targets,
    data_labels=_data_targets,
  )
  # Run k8s_object with the generated secret
  k8s_object(
    name = name,
    kind = "secret",
    template = name + "_secret.yaml",
    cluster = cluster,
    namespace = namespace,
    **kw)
