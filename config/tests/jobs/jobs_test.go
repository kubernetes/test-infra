	...
	for _, c := range spec.Containers {
		for _, r := range resourceNames {
			...
			if limit.Cmp(zero) == 0 {
				...
				// Enforce a maximum memory limit to allow jobs to fit in an 8 core, 31G Node
				maxMem := resource.MustParse("28Gi")
				...
				// Enforce a maximum CPU limit to allow jobs to fit in an 8 core, 31G Node
